...
function ChipGroup({ opts, val, onChange, onFirstPick, showOtro = false, otroVal = "", onOtroChange }: {
  opts: string[]; val: string; onChange: (v: string) => void;
  onFirstPick: (x: number, y: number) => void;
  showOtro?: boolean; otroVal?: string; onOtroChange?: (v: string) => void;
}) {
  const [otroFocused, setOtroFocused] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => { if (val === "Otro") setTimeout(() => inputRef.current?.focus(), 50); }, [val]);
  return (
    <div>
      ...
      {showOtro && val === "Otro" && (
        <div style={{ marginTop: 10 }}>
          <input ref={inputRef} type="text" placeholder="¿Qué taller tomaste?" value={otroVal}
            onChange={(e) => onOtroChange?.(e.target.value)}
            onFocus={() => setOtroFocused(true)} onBlur={() => setOtroFocused(false)}
            style={{
              width: "100%", borderRadius: 10, padding: "13px 16px",
              fontFamily: "'Satoshi',sans-serif", fontSize: 16, fontWeight: 300, color: T.ink,
              outline: "none", boxSizing: "border-box" as const,
              background: otroFocused ? T.white : "rgba(252,250,247,1)",
              border: otroFocused ? `2px solid ${T.green}` : `1.5px solid ${T.border}`,
              boxShadow: otroFocused ? "0 0 0 3px rgba(18,83,92,0.08)" : "none",
              transition: "border-color 0.2s, box-shadow 0.2s",
            }}
          />
        </div>
...
export default function SatisfaccionTalleresPage() {
  ...
  const [form, setForm] = useState({
    nombre: "", taller: "", tallerOtro: "", experiencia: 0, instructor: 0,
    acompanamiento: "", ritmo: "", materiales: "", comodidad: "",
    lo_mejor: "", mejoras: "", regresar: "", nuevos_talleres: "",
  ...
  const tallerEnvio = form.taller === "Otro"
    ? (form.tallerOtro.trim() ? `Otro: ${form.tallerOtro.trim()}` : "Otro")
    : form.taller;
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.taller) { setError("Por favor indica el taller que tomaste."); return; }
    ...
    try {
      const res = await fetch(FORMSPREE_URL, {
        ...
        body: JSON.stringify({
          _subject: `Nueva respuesta de ${form.nombre || "Anónimo"} — Satisfacción talleres Papela`,
          Nombre: form.nombre || "Anónimo",
          "Taller tomado": tallerEnvio,
          "Experiencia general": `${form.experiencia}/5 ★`,
          "Claridad del instructor": `${form.instructor}/5 ★`,
          ...
          "Talleres sugeridos": form.nuevos_talleres || "—",
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 17, lineHeight: "26px", fontWeight: 400, color: T.inkSoft, maxWidth: 340, margin: "0 auto" }}>
          Tu opinión nos ayuda a crear talleres más bonitos, creativos y especiales para ti.
        </p>
      ...
      <div style={{ width: "100%", maxWidth: 680, display: "flex", flexDirection: "column", alignItems: "center" }}>
        ...
        <section style={{ width: "100%", textAlign: "center", padding: "64px 0 48px" }}>
          ...
          <h1 style={{
            ...
            Queremos mejorar<br />
            <em style={{ fontStyle: "italic", color: T.terracota }}>tus talleres en Papela</em>
          </h1>
          <p style={{ fontSize: 17, lineHeight: "26px", fontWeight: 400, color: T.inkSoft, maxWidth: 460, margin: "0 auto 18px" }}>
            Gracias por ser parte de nuestros talleres. Tu opinión nos ayuda a mejorar la experiencia, el espacio y la forma en que acompañamos tu proceso creativo.
          </p>
        ...
        <div style={{
          ...
          <form onSubmit={submit} noValidate>
            ...
            <Fld l="¿Qué taller tomaste en Papela? *">
              <ChipGroup opts={TALLERES} val={form.taller}
                onChange={(v) => { set("taller")(v); if (v !== "Otro") set("tallerOtro")(""); }}
                onFirstPick={(x, y) => once("taller", x, y)}
                showOtro otroVal={form.tallerOtro} onOtroChange={(v) => set("tallerOtro")(v)} />
            </Fld>
            ...
            <Fld l="¿El instructor explicó de forma clara?">
              <Stars val={form.instructor} onChange={set("instructor")} lo="Nada claro" hi="Muy claro" onPick={onStarPick} />
            </Fld>
            <Fld l="¿Sentiste que recibiste suficiente acompañamiento durante el taller?">
            ...
            <Fld l="¿El ritmo del taller te pareció adecuado?">
            ...
            <Fld l="¿Cómo te sentiste durante el taller?">
            ...
            <Sec c="Tu opinión" />
            <Fld l="¿Qué fue lo que más te gustó del taller?">
            ...
            <Fld l="¿Te gustaría seguir tomando talleres en Papela?">
              <ChipGroup opts={["Sí", "No", "Tal vez"]} val={form.regresar}
                onChange={(v) => set("regresar")(v)} onFirstPick={(x, y) => once("regresar", x, y)} />
            </Fld>
            <Fld l="¿Qué tipo de talleres te gustaría que agregáramos?">
              <TArea placeholder="Ej. Acuarela, cerámica, bordado, lettering, fotografía..." value={form.nuevos_talleres}
                onChange={(e) => set("nuevos_talleres")(e.target.value)} onBlur={(e) => onBlurText("nuevos_talleres", e)} />
            </Fld>

            <Sec c="Recomendación" />
            <Fld l="¿Recomendarías nuestros talleres a alguien más?">
              <ChipGroup opts={["Sí", "No", "Tal vez"]} val={form.recomendacion}
                onChange={(v) => set("recomendacion")(v)} onFirstPick={(x, y) => once("recomendacion", x, y)} />
            </Fld>
            <Fld l="Comentario extra o mensaje para el instructor">
