...
| `TarjetaClub` | `components/site/TarjetaClub.tsx` | **Tarjeta del Club Creativo** (`/club/[token]`, ruta standalone en `ConditionalShell` + `noindex`). Los datos viven en el admin: `GET/PATCH https://admin.papela-atelier.com/api/public/club/<token>` + catálogo `GET .../club/stickers` (CORS abierto). Pase vertical estilo wallet: título foil + **álbum de 100 coleccionables** (contador X/100, preview de 6 espacios con hitos, CTA "🎁 Rascar" si hay pendientes, "Ver mi planilla completa"), nombre foil + fecha, talón punteado con nº de socio + QR client-side (`qrcode`), botón compartir. Personalización (16 fondos con deriva animada + grano, 7 texturas, holo, 4 charms arrastrables) → PATCH con whitelist en el admin. Tilt 3D + `prefers-reduced-motion` |
| `PlanillaClub` | `components/site/club/PlanillaClub.tsx` | Planilla completa del álbum (overlay full-screen): 100 espacios — obtenidos a color con badge ×N y **Regalar** (duplicados → código de un solo uso, 72h, share por WhatsApp), definidos-no-obtenidos como silueta, resto "?"; chips de hitos de recompensa; input para **canjear código** de regalo de otro miembro |
