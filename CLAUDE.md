...
| `TarjetaClub` | `components/site/TarjetaClub.tsx` | **Tarjeta del Club Creativo** (`/club/[token]`, ruta standalone en `ConditionalShell` + `noindex`). Los datos viven en el admin: `GET/PATCH https://admin.papela-atelier.com/api/public/club/<token>` + catálogo `GET .../club/stickers` (CORS abierto). Pase vertical estilo wallet: título foil + **álbum de 100 coleccionables** (contador X/100, preview de 6 espacios con hitos, CTA "🎁 Rascar" si hay pendientes, "Ver mi planilla completa"), nombre foil + fecha, talón punteado con nº de socio + QR client-side (`qrcode`), botón compartir. Personalización (16 fondos con deriva animada + grano, 7 texturas, holo, 4 charms arrastrables) → PATCH con whitelist en el admin. Tilt 3D + `prefers-reduced-motion`. **Campana de notificaciones** (top-right): aviso de stickers pendientes + mensajes del negocio (`GET .../club/mensajes`); la pill verde de "por revelar" se descarta con ✕ (persistido en localStorage por token, reaparece si cambia el conteo) y todo sigue visible en la campana. Fix iOS: `isolation:isolate` + `-webkit-mask-image` en el contenedor de la tarjeta (esquinas redondeadas vs hijos con transform animado) |
| `PlanillaClub` | `components/site/club/PlanillaClub.tsx` | Planilla SIMPLE (mockup jul-2026): flecha atrás + "Mi planilla Papela" + X/100 y la cuadrícula. Obtenidos a color con ×N (sin etiqueta "Común"; raros/legendarios con **marco holográfico animado** `.club-marco-holo`); no obtenidos = imagen real fantasma (opacity .18) + N.º; sin definir = "?". **Tap en cualquier definido → ficha** (bottom sheet): historia + premio descargable (solo dueños, vía `stickers.detalles` del GET por token) + **Regalar** repetidos (código 72h, WhatsApp); versión misteriosa para no obtenidos. Canje de código de regalo como link discreto al pie |
